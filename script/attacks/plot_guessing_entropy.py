...
def load_sca_model(model_file):
    ...
    try:
        model = load_model(model_file, custom_objects={'mean_rank': mean_rank})
        print('Ok')
    except:
...
def rank(predictions, metadata, real_key, min_trace_idx, max_trace_idx, last_key_bytes_proba, predictor):
    # Compute the rank
    ...
    for p in range(0, max_trace_idx-min_trace_idx):
        ...
        ## ADDING
        if (predictor == "SNR4") or (predictor == "SNR5"):
            mask = metadata[min_trace_idx + p]['masks'][2]
        elif (predictor == "SNR2") or (predictor == "SNR3"):
            mask = metadata[min_trace_idx + p]['masks'][-1]
        ...
        for i in range(0, 256):
            ...
            ## ADDING
            if (predictor == "SNR4") or (predictor == "SNR2"):
                proba = predictions[p][AES_Sbox[plaintext ^ i]^mask]
            elif predictor == "SNR1":
                proba = predictions[p][AES_Sbox[plaintext ^ i]]
            #elif (predictor == "SNR3") or (predictor == "SNR5"):
...
def full_ranks_perso(model, dataset, metadata, min_trace_idx, max_trace_idx, rank_step, predictor):
    ...
    for t, i in zip(index, range(0, len(index))):
        real_key_rank, key_bytes_proba = rank(predictions[t-rank_step:t], metadata, real_key, t-rank_step, t, key_bytes_proba, predictor)
